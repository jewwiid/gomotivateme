...
export function HomeContent({ initialRecent }: { initialRecent: HomeGoal[] }) {
  const { user } = useCurrentUser();
  const router = useRouter();
  const liveRecent = useQuery(api.public.listRecentPublic, { limit: 12 });
  const recent = (liveRecent ?? initialRecent) as HomeGoal[];
