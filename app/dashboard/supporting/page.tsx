      ...
      <main className="shell-content">
        ...
        {supports === undefined ? (
          <div className="workspace-card mt-7 divide-y divide-[var(--color-border)] overflow-hidden">
            {[0, 1, 2].map((i) => (
              <div key={i} className="h-24 animate-pulse bg-[var(--color-bg-elev)]" />
            ))}
          </div>
        ) : supports.length === 0 ? (
          <div className="workspace-card mt-7 grid place-items-center border-dashed px-6 py-16 text-center">
            <Heart size={28} className="text-[var(--color-gold-text)]" />
                ...
                >
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-[var(--color-text-muted)]">
                    ...
                    {s.goalStatus === "paused" && (
                      <>
                        <span className="h-4 w-px bg-[var(--color-bg-sunken)]" />
                        <span className="text-[var(--color-text-dim)]">Paused</span>
                      </>
                    )}
                  </div>
                  {s.pledge && (
                    <p className="mt-2 line-clamp-1 text-sm italic text-[var(--color-text-muted)]">
                      "{s.pledge}"
                    </p>
                  )}
                </Link>
